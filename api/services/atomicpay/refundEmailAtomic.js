...
async function sendRefundEmailAtomic(email, nome = "Tutor(a)") {
  const html = `
    <div style="font-family: Inter, sans-serif; color: #1C1C1C; padding: 20px;">
      ...
      <p>O estorno será concluído dentro dos próximos dias, conforme a política da plataforma e do seu método de pagamento.</p>
  ...
  try {
    ...
    console.log("✅ Refund e-mail (Atomic) enviado com sucesso!", response.status);
  } catch (err) {
    console.error("❌ Erro ao enviar Refund e-mail (Atomic):", err.response?.data || err.message);
  }
}

module.exports = sendRefundEmailAtomic;
